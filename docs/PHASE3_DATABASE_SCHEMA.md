...
- **`athlete_profiles.id = auth.users.id` (§5.1) noch einmal bewusst prüfen, bevor das echte Schema steht.** Diese harte 1:1-Kopplung verhindert strukturell, dass ein Shooter existieren kann, ohne bereits einen Account zu haben — relevant sowohl für Walk-up-Registrierungen durch den Match Director (jetzt schon denkbar) als auch für später importierte/unclaimed Profile (siehe [ROADMAP_EXTENSIONS.md](./ROADMAP_EXTENSIONS.md) Punkt 4). `officials.user_id` ist im selben Schema bereits nullable, genau für einen analogen Fall (RO ohne vollen Account) — dasselbe Muster (`competitors`/`shooters`-Entität mit optionalem `user_id`) verallgemeinert sich direkt auf Athleten. Kein Implementierungsauftrag, nur: diese Entscheidung nicht versehentlich ein zweites Mal hart verdrahten.
